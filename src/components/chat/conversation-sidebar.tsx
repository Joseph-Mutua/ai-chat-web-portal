...
export function ConversationSidebar({ isOpen = true, onClose }: ConversationSidebarProps) {
  ...
  const { data, isLoading, fetchNextPage, hasNextPage, isFetchingNextPage } = useChatConversations({
    page: 1,
    limit: 20,
  })
      ...
      >
        <div className="p-5 flex items-center justify-between">
          <div className="hidden lg:flex items-center gap-3">
            <Image src={logoImage} alt="warpSpeed" width={32} height={32} className="w-8 h-8 object-contain" />
        ...
        <nav className="px-4 flex-1" role="navigation" aria-label="Main navigation">
          <button
          ...
          {showHistory && (
            <div id="chat-history-list" className="ml-4 mt-2 max-h-[300px] overflow-y-auto" role="list" aria-label="Chat history">
              {isLoading ? (
                  ...
                  {Object.entries(groupedConversations).map(([date, convos]) => (
                    <div key={date}>
                      ...
                      {convos.map((conversation) => {
                        ...
                        const title = conversation.title || conversation.lastMessage || firstMessagesMap[conversation.id] || null
                        return (
                          <button
                            key={conversation.id}
            ...
            <div className="flex justify-center mb-3">
              <div className="relative">
                <svg width="56" height="56" viewBox="0 0 56 56" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <path d="M10 6H46L52 26L28 52L4 26L10 6Z" fill="url(#gemGradient)" />
                  <path d="M10 6H46L52 26H4L10 6Z" fill="rgba(255,255,255,0.12)" />
                  ...
                  <path d="M10 6L4 26L28 52V6H10Z" fill="rgba(0,0,0,0.04)" />
                  <g transform="translate(16, 14)">
                    <path d="M4 14L2 5L7 9L12 4L17 9L22 5L20 14H4Z" fill="white" />
                    <rect x="4" y="15" width="16" height="3" rx="1" fill="white" />
                  </g>
                </svg>
              </div>
            </div>
            <h3 className="text-center font-semibold text-xl mb-1 text-base">Upgrade Your Plan</h3>
...
function groupConversationsByDate(conversations: IChatSession[]) {
  ...
  conversations.forEach((conv) => {
    ...
    else if (diffDays < 30) label = date.toLocaleDateString('en-US', { month: 'long', day: 'numeric' })
